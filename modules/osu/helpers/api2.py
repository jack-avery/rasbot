...
class OsuAPIv2Helper(Singleton):
    ...
    def __get_auth(self):
        ...
        link = link.replace("%client_id%", self.cfg["client_id"]).replace(
        ...
        logger.info("Success! rasbot osu! modules using the v2 API will now work.")
    ...
    def __get_token(self, data):
        ...
        token = post("https://osu.ppy.sh/oauth/token", headers=headers, json=data)
    ...
    def __request(self, method, endpoint: str, data: dict = None):
        ...
        headers[
            "Authorization"
        ] = f"{self.cfg['token']['token_type']} {self.cfg['token']['access_token']}"
        if data:
            response = method(endpoint, headers=headers, json=data)
        else:
            response = method(endpoint, headers=headers)
